            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('users.update', $user->id) }}">
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <label for="expires" class="block font-medium text-sm text-gray-700">Expires</label>
                            <input type="date" name="expires" id="expires" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('expires', $user->expires) }}" defaulValue=""/>
                            @error('expires')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
